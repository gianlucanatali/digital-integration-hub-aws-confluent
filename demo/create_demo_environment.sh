...
function init_vars_from_config() {
    ...
    #     ;; 
    aws)
        if [[ -z $AWS_REGION || -z $AWS_ACCESS_KEY || -z $AWS_SECRET_KEY ]]; then
            echo "one or more variables are undefined, check your $CFG_FILE "
            echo "required variables: AWS_REGION, AWS_ACCESS_KEY, AWS_SECRET_KEY "
        fi
        ;; 
     *)
        echo "Use one of the CLOUD_PROVIDER: aws "
        exit 1
        ;;
    esac
